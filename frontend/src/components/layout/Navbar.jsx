...
import { BookOpen, ShieldCheck, Trophy, Gamepad2, FlaskConical } from 'lucide-react';
  ...
  return (
    <nav className="sticky top-0 z-50 flex h-[64px] items-center border-b-2 border-ink bg-paper/85 px-6 backdrop-blur-md">
      ...
      <div className="flex gap-2">
        ...
        {/* Public, like the landing page: contest entrants who have not taken
            the course still need the manual. */}
        <Link
          to="/manual"
          aria-label="Game manual"
          className="flex items-center gap-1.5 rounded-lg px-3 py-2 font-bold text-ink/70 transition-colors hover:bg-pcb/10 hover:text-pcb"
        >
          <Gamepad2 className="h-4 w-4" /> <span className="hidden sm:inline">Manual</span>
        </Link>
        <Link
          to="/virtual-lab"
          ...
          className="flex items-center gap-1.5 rounded-lg px-3 py-2 font-bold text-ink/70 transition-colors hover:bg-pcb/10 hover:text-pcb"
        >
          <FlaskConical className="h-4 w-4" /> <span className="hidden sm:inline">Virtual Lab</span>
        </Link>
        {user && (
          <Link
            to="/contests"
